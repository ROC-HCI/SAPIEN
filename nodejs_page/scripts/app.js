...
function showConnectOverlay() {
    ...
    startText.style.position = 'relative';

    let textDiv = document.createElement('div');

    textDiv.style.zIndex = '3';
    textDiv.style.position = 'fixed';  // It should be positioned absolutely within startText
    textDiv.style.left = '0';  // Align text to the left of startText
    textDiv.style.width = '100vw';
    textDiv.style.textAlign = 'center';  // Center align the text
    textDiv.style.transform = 'translateY(-150%)'; // Adjust the position
    ...
    callText.style.fontSize = '1.5em';
    callText.style.fontWeight = 'bold';
    callText.style.color = 'white';
    callText.style.fontFamily = 'Michroma';
    callText.style.whiteSpace = 'pre'; // Preserve whitespace
    ...
    callText.appendChild(dot1);
    callText.appendChild(dot2);
    callText.appendChild(dot3);

    function addDot() {
        counter = (counter + 1) % 4;
        dot1.style.visibility = (counter > 0) ? 'visible' : 'hidden';
        dot2.style.visibility = (counter > 1) ? 'visible' : 'hidden';
        dot3.style.visibility = (counter > 2) ? 'visible' : 'hidden';
    }

    setInterval(addDot, 1000);

    textDiv.appendChild(callText);

    startText.innerHTML = `<div class="unique-incoming-call-animation">
    <style>
        :root {
            --size: 20px;
        }

      .unique-incoming-call-animation {
        position: relative;
      }

      :root {
        --unique-pink: #055c13;
        --unique-animation-curve: cubic-bezier(0.12, 0.7, 0.74, 0.71);
      }
      ...
      .unique-incoming-call-animation .unique-phone {
        z-index: 2;
        background-color: var(--unique-pink);
        animation: unique-phone 1s var(--unique-animation-curve) infinite alternate-reverse;
        display: grid;
      ...
      .unique-incoming-call-animation .material-icons {
        color: white;
        font-size: .5em;
      }

      .unique-incoming-call-animation .unique-circle {
        height: var(--size);
        width: var(--size);
        border-radius: 50%;
        background-color: red;
        position: absolute;
        animation: unique-scaling 2s var(--unique-animation-curve) infinite;
      }

      @keyframes unique-scaling {
        0% {
          transform: scale(4);
          background-color: var(--unique-pink);
        }
      ...
      .unique-incoming-call-animation .unique-circle1 {
        animation-delay: 0s;
      }

      .unique-incoming-call-animation .unique-circle2 {
        animation-delay: 1s;
      }
    </style>
    ...
    <div class="unique-circle unique-circle1"></div>
    <div class="unique-circle unique-circle2"></div>
  </div>`;

    startText.appendChild(textDiv);

    const acceptText = document.createElement('p');
    acceptText.id = 'acceptText';
    acceptText.style.textAlign = 'center';
    acceptText.style.fontSize = '0.75em';
    acceptText.style.marginTop = '-25px';
    acceptText.innerHTML = 'Click to accept';
    
    textDiv.appendChild(acceptText);

    setOverlay('clickableState', startText, event => {
        connect();
        startAfkWarningTimer();
        // make video element take up whole page
...
function playStream() {
    ...
    if (webRtcPlayerObj && webRtcPlayerObj.video) {
        ...
        wait(1).then(() => {
            ...
            wait(2000).then(() => {  // Loading screen timer
...
function showPlayOverlay() {
    console.log("At showPlayOverlay")
    ...
    console.log("shouldShowPlayOverlay is set to " + shouldShowPlayOverlay)
    // HERE THE BUTTONS SHOULD START SHOWING UP
}
